// ============================================================
// LajuAman — Bus Simulator Cilik (Revised Full Overlay)
// ============================================================
...
const BusGame = (function() {
    ...
    function drawScene() {
        ...
        for (let i = -1; i < 2; i++) {
            ...
            drawRoundedRect(ctx, centerX - 290, yPos + 260, 24, 8, 3);
            ctx.fill();
            drawRoundedRect(ctx, centerX + 270, yPos + 350, 20, 7, 2);
            ctx.fill();
        }
        ...
        roadEvents.forEach(ev => {
            ...
            if (screenY > -200 && screenY < canvas.height + 200) {
                ...
                if (ev.type === 'BUS_STOP') {
                    ...
                    ctx.beginPath();
                    ctx.arc(centerX + 87, screenY - 30, 12, 0, Math.PI * 2);
                    ctx.fill();
                    ...
                    ctx.fillStyle = '#0f172a';
                    drawRoundedRect(ctx, centerX - 116, screenY - 40, 30, 65, 8);
                    ctx.fill();

                    ctx.fillStyle = (ev.lightState === 'RED') ? '#ef4444' : '#1e293b';
                    ctx.beginPath();
                    ctx.arc(centerX - 101, screenY - 25, 7, 0, Math.PI * 2);
                    ctx.fill();
                    if (ev.lightState === 'RED') {
                        ...
                        ctx.beginPath();
                        ctx.arc(centerX - 101, screenY - 25, 4, 0, Math.PI * 2);
                        ctx.fill();
                        ctx.shadowBlur = 0;
                    }
                    ...
                    ctx.beginPath();
                    ctx.arc(centerX - 101, screenY - 8, 7, 0, Math.PI * 2);
                    ctx.fill();

                    ctx.fillStyle = (ev.lightState === 'GREEN') ? '#22c55e' : '#1e293b';
                    ctx.beginPath();
                    ctx.arc(centerX - 101, screenY + 9, 7, 0, Math.PI * 2);
                    ctx.fill();
                    if (ev.lightState === 'GREEN') {
                        ...
                        ctx.beginPath();
                        ctx.arc(centerX - 101, screenY + 9, 4, 0, Math.PI * 2);
                        ctx.fill();
                        ctx.shadowBlur = 0;
                    }
                    ...
                    ctx.beginPath();
                    ctx.arc(centerX - 100, screenY - 40, 18, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.fillStyle = '#ffffff';
                    ctx.beginPath();
                    ctx.arc(centerX - 100, screenY - 40, 13, 0, Math.PI * 2);
                    ctx.fill();
                    ...
                    ctx.fillStyle = '#eab308';
                    drawRoundedRect(ctx, centerX + 85, screenY - 50, 24, 20, 4);
                    ctx.fill();
                    ctx.fillStyle = '#0f172a';
                    ctx.beginPath();
                    ctx.arc(centerX + 97, screenY - 40, 5, 0, Math.PI * 2);
                    ctx.fill();
                    ...
                    drawRoundedRect(ctx, -20, 0, 40, 14, 4);
                    ctx.fill();
                    ...
                    ctx.beginPath();
                    ctx.ellipse(-20, 7, 3, 7, 0, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.beginPath();
                    ctx.ellipse(20, 7, 3, 7, 0, 0, Math.PI * 2);
                    ctx.fill();
    ...
    function drawBus() {
        ...
        ctx.beginPath();
        ctx.arc(-BUS.width / 2 + 6, -BUS.height / 2 + 2 + BUS.suspensionOffset, 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(BUS.width / 2 - 6, -BUS.height / 2 + 2 + BUS.suspensionOffset, 3, 0, Math.PI * 2);
        ctx.fill();
    ...
    function updateGame(delta) {
        ...
        if (speedoContainer) {
            if (BUS.currentMaxSpeed < BUS.maxSpeed) {
                ...
                // Kecepatan normal
                speedoContainer.className =
                    "bg-slate-950/50 px-3.5 py-1.5 rounded-2xl flex flex-col items-center justify-center border border-white/10 min-w-[85px] shadow-inner transition-colors duration-300";
    ...
    function processRoadEvents(dt) {
        roadEvents.forEach(ev => {
                    ...
                    if (tutorialBubble) {
                        if (ev.name === "Halte Sakura") {
                            tutorialBubble.innerHTML =
                                "💡 <span class='text-emerald-400 font-bubble'>Sinyal Lampu Merah:</span> Hati-hati, ada <b class='text-red-400'>Lampu Merah</b> menanti di depan. Berhentilah tepat sebelum garis putih!";
                        } else {
                            tutorialBubble.innerHTML =
                                "💡 <span class='text-emerald-400 font-bubble'>Gerbang Sekolah:</span> Kerja luar biasa! Melajulah perlahan sampai tiba di pintu masuk gerbang sekolah 🏫!";
                        }
                    }
                }
            }
            ...
            // --- EVENT: LAMPU MERAH-HIJAU LALU LINTAS ---
                ...
                if (distanceToBus < -100 && !ev.cleared && tutorialBubble) {
                    ev.cleared = true;
                    tutorialBubble.innerHTML =
                        "💡 <span class='text-emerald-400 font-bubble'>Ambulans Belakang:</span> Dengarkan bunyi sirene! Berpindahlah ke <b class='text-emerald-400'>LAJUR KIRI</b> untuk memberi jalan bagi Ambulans Gawat Darurat!";
                }
            }
            ...
            // --- EVENT: RADAR SIRENE AMBULANS ---
                ...
                if (distanceToBus <= 150) {
                    ev.cleared = true;
                    // Spawn Ambulans dari bawah layar
            ...
            // --- EVENT: BATAS KECEPATAN (ZONA SEKOLAH) ---
                ...
                if (distanceToBus <= 300 && distanceToBus >= -50) {
                    ...
                    if (tutorialBubble) {
                        tutorialBubble.innerHTML =
                            "💡 <span class='text-yellow-400 font-bubble'>Tips Kamera Tilang:</span> Kamera tilang tidak bisa dihindari dengan ganti lajur! Anda wajib mengerem di bawah <b>30 km/h</b>.";
                    }
                }
                if (distanceToBus < -150) {
                    ev.cleared = true;
            ...
            // --- EVENT: PENYEBERANG SATWA LIAR (Kucing Gurun) ---
                ...
                if (distanceToBus <= 250 && !catHomed && tutorialBubble) {
                    tutorialBubble.innerHTML =
                        "💡 <span class='text-yellow-400 font-bubble'>Penyelamat Satwa:</span> Ada anak kucing di jalan! Bunyikan <b class='text-blue-400'>KLAKSON</b> agar ia lari atau berhenti total!";
                }
                ...
                if (distanceToBus < -150 && tutorialBubble) {
                    ev.cleared = true;
                    tutorialBubble.innerHTML =
                        "💡 <span class='text-emerald-400 font-bubble'>Rintangan Kayu:</span> Ada pohon tumbang menutup lajur kanan di depan. Kemudikan bus ke <b class='text-yellow-400'>LAJUR KIRI</b>!";
                }
            }
            ...
            // --- EVENT: RINTANGAN BATANG KAYU (Lajur Kanan) ---
                ...
                if (distanceToBus < -100 && !ev.cleared && tutorialBubble) {
                    ev.cleared = true;
                    tutorialBubble.innerHTML =
                        "💡 <span class='text-emerald-400 font-bubble'>Swerve Sukses:</span> Refleks luar biasa! Anda berhasil bergeser lajur menghindari hambatan jalan.";
                }
            }
        });
    }
    ...
    function drawOverlayFeedback() {
        if (warningTimer > 0 && warningText) {
            ctx.save();
            ctx.font = 'bold 13px Quicksand, Arial, sans-serif';
            ctx.textAlign = 'center';
            const centerX = canvas.width / 2;
            ...
            if (warningText.includes("POIN") || warningText.includes("MENABRAK") || warningText.includes("TILANG")) {
                ...
                drawRoundedRect(ctx, boxX, 160, boxWidth, 50, 12);
                ctx.fill();
                ctx.fillStyle = '#f87171';
    ...
    function handleSuccessArrival() {
        ...
        if (BUS.passengers >= 4) {
            score += 5;
            winText =
                "LUAR BIASA SUPIR TELADAN! Anda sukses menjemput semua anak sekolah, tertib menaati limit tilang 30 km/h, dan menyelamatkan hewan jalanan. Poin bonus +5 Bintang! 🌟";
        } else {
            score += 2;
            winText =
                "Perjalanan selesai! Di rute selanjutnya, pastikan Anda berhenti dengan lembut di setiap halte bus agar tidak ada anak sekolah yang tertinggal ya! 😊";
        }

        scoreDisplay.textContent = `Bintang: ${score}`;
        const winDescEl = document.getElementById('bus-win-description');
        if (winDescEl) winDescEl.textContent = winText;
    ...
    return {
        init: () => {
            ...
            if (scoreDisplay) scoreDisplay.textContent = `Bintang: 0`;
            if (pointsDisplay) pointsDisplay.textContent = `Poin SIM: 3`;
            if (speedoDisplay) speedoDisplay.textContent = `0 km/h`;
